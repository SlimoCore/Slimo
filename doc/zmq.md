...
The Slimo Core daemon can be configured to act as a trusted "border
router", implementing the slimo wire protocol and relay, making
...
The ZeroMQ feature in Slimo Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building slimod:
...
For instance:
    ...
    $ slimod -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/slimod.tx.raw \
...
These options can also be provided in slimo.conf.
...
From the perspective of slimod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into slimod directly. Furthermore, no information is
...
using. Slimod appends an up-counting sequence number to each
