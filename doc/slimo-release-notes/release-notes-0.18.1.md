Slimo Core version 0.18.1 is now available from:

  <https://download.slimo.org/slimo-0.18.1/>
...
`/Applications/Slimo-Qt` (on Mac) or `slimod`/`slimo-qt` (on
Linux).
...
Slimo Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Slimo Core on unsupported systems.
...
10.10. Additionally, Slimo Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:20769:20769` (this is an extra `:20769` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@slimo.org) the Slimo Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 SLMO),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in SLMO per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/slimo-conf.md)
  about the `slimo.conf` file describes how to use it to configure
  Slimo Core.

- A new document introduces Slimo Core's BIP174 [Partially-Signed
  Slimo Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Slimo-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Slimo Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Slimo Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Slimo Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart slimod with the `-deprecatedrpc=generate`
...
- A new `slimo-wallet` tool is now distributed alongside Slimo
...
This section describes planned changes to Slimo Core that may affect
other Slimo software and services.

- Since version 0.16.0, Slimo Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Slimo Core 0.20 (expected about
  a year after 0.18), Slimo Core will default to native segwit
  ...
  to bech32 addresses, and if the Slimo Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Slimo Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Slimo Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Slimo Core
...
- A sub-project of Slimo Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Slimo Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Slimo Core's own implementation, although entropy
  gathered by Slimo Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Slimo Core a
...
- On macOS, Slimo Core now opts out of application CPU throttling
...
- #20769 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
